...
The pure v1 resolver still validates tenant and instance overrides. Panda maps approved semantic colours to its validated `--tocyn-color-*` properties with static fallbacks, while the authenticated dashboard applies the tenant palette and operator-selected mode through [`OperatorThemeProvider`](../apps/dashboard/src/components/theme/OperatorThemeProvider.tsx) and `createTocynThemeScope`. The operator preference is separate from tenant branding. The portal aligns its document-root light/dark class with its auth shell so Park palette aliases resolve in the same mode. The widget currently mounts a light Park palette and permits only a validated tenant brand colour for its panel accent; it does not restore an operator preference. The widget [specification](phase-1.8-widget-spec.md) names a `LUMINA_WIDGET_CSS` compatibility hook, but current widget source does not read that hook; compatibility needs an explicit decision before #315 acceptance.

This is a source and local-check checkpoint, not final visual or release acceptance. Route-by-route browser evidence, light/dark contrast, keyboard focus, reduced motion, 200% zoom, VoiceOver, exact-head CI/CodeQL and widget compatibility remain to be closed for PR #316. The contract table above states 4.5:1 text contrast, while the current `resolveTocynTheme` validation rejects values below 4.3:1; that difference also needs correction or an approved contract decision. The #66 evidence above stays attached to its historical revision and does not clear these #315 gates.
