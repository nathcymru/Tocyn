...
Historical receipt. The 12 September owner instruction under #258 supersedes this
record's Spark-first recommendation: use [current worker routing](../skills/tocyn-worker-routing/SKILL.md)
and [the resource snapshot](../resources/development-workers.md). Capacity/reset
figures and pending probes below describe that earlier session, not current availability.
...
## Global inherited Graphify configuration — 11 September, 21:00 BST

The same read-only Graphify configuration is now present in the standalone Codex user configuration at `/Users/ty/.codex/config.toml`, including `approve` only for `list_repositories` and `query_graph`. `codex mcp get graphify` from `/tmp` verified that an isolated working directory inherits the server. This removes the worktree-configuration gap. An actual Spark Graphify query remains pending the five-hour window reset; do not claim that query has occurred.
