...
The next bounded server slice is local and unpushed on the same branch. `NotificationDO` accepts only the versioned client ticket/revision/active payload, derives the actor from the live staff session and authorizes the sender plus every recipient against the current tenant ticket and group membership. It limits each transient connection/ticket cadence to one update per second, retains no typing state across close, reconnect or hibernation, sends a six-second server expiry, and remains within the existing 128-socket ceiling. The new disposable Miniflare proof exercises two tenants, authorized and denied same-tenant recipients, a customer denial, malformed input, membership revocation, throttle, expiry and reconnect; the existing 128-capacity native proof also passes. The complete 12-test local storage/background runtime command passes on Node 22.19.0 using the declared Zod 3.25.76 through an ignored worktree-local `apps/server/node_modules/zod` alias; the earlier Zod 4 resolution was dependency setup drift, not a #64 resource defect. It is advisory only and does not create typing replay or a second realtime store.
