...
5. Before review, complete implementation, relevant deterministic checks and substantive coordinator review. Follow the AGENTS.md scarce-Copilot gate: zero default; essential unresolved code/security risk only, one justified consolidated request at the final merge-ready head, never a draft/per-push loop or approval-count workaround. Any repeat needs explicit maintainer authority for a new unresolved material risk. Keep manual-only automatic-review settings, required security/CI and the standing PR-only owner bypass rules.
