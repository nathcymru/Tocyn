...
const server=await createServer({logLevel:"error",root:join(root,'tools/ui-browser/fixture'),configFile:false,server:{host:'127.0.0.1',port:0,fs:{allow:[root]}},plugins:[{name:'local-shared-style',configureServer(server){server.middlewares.use('/panda.css',(_req,res)=>{res.setHeader('Content-Type','text/css');res.end(readFileSync(join(root,'packages/ui/src/styles/panda.css')));});}}]});
let browser;
try{
 ...
 for(const style of ['default','radical','none']){
  ...
  try{
   ...
   await page.getByLabel('Subject',{exact:true}).fill('Synthetic subject');await page.getByRole('combobox',{name:'Priority'}).click();await page.getByRole('option',{name:'High',exact:true}).click();await page.getByRole('textbox',{name:'Details',exact:true}).fill('Synthetic body');
   ...
   const combo=page.getByRole('combobox',{name:'Find view'});await combo.focus();await page.keyboard.press('ArrowDown');await page.getByRole('option',{name:'Mine',exact:true}).last().waitFor();await page.keyboard.press('End');await page.keyboard.press('Enter');await page.waitForFunction(()=>document.querySelector('output[aria-label="Chosen view"]')?.textContent==='Needs Action');assert.equal(await combo.inputValue(),'Needs Action');await page.waitForFunction(()=>document.querySelector('[role=combobox]')?.getAttribute('aria-expanded')==='false');
   await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
   const splitter=page.getByRole('separator',{name:'Resize navigation'});await splitter.focus();await page.waitForFunction(()=>document.activeElement?.getAttribute('role')==='separator' && document.activeElement.hasAttribute('data-focus'));const before=Number(await splitter.getAttribute('aria-valuenow'));await page.keyboard.press('ArrowRight');await page.waitForFunction(old=>Number(document.querySelector('[role=separator]')?.getAttribute('aria-valuenow'))>old,before);await splitter.press('End');await page.waitForFunction(()=>Number(document.querySelector('[role=separator]')?.getAttribute('aria-valuenow'))>=79);const end=Number(await splitter.getAttribute('aria-valuenow'));assert.ok(end<=80);await page.keyboard.press('Home');await page.waitForFunction(()=>Number(document.querySelector('[role=separator]')?.getAttribute('aria-valuenow'))<=21);assert.ok(Number(await splitter.getAttribute('aria-valuenow'))>=20);
   ...
   const sizes=await page.locator('.button,.input,.textarea,[data-scope=select][data-part=trigger]').evaluateAll(elements=>elements.filter(el=>!el.closest('[hidden]')&&el.getBoundingClientRect().width>0).map(el=>({kind:el.getAttribute('data-part')??el.tagName.toLowerCase(),width:el.getBoundingClientRect().width,height:el.getBoundingClientRect().height})));
   let presentation='not asserted without CSS';
   if(style!=='none'){const minimum=40;for(const size of sizes)assert.ok(size.height>=minimum,`${style} ${size.kind} target height ${size.height} < ${minimum}`);const motion=await page.locator('[data-motion-probe]').evaluate(el=>({transition:getComputedStyle(el).transitionDuration,animation:getComputedStyle(el).animationDuration}));assert.ok(Number.parseFloat(motion.transition)<=0.001 && Number.parseFloat(motion.animation)<=0.001,`Reduced motion durations: ${JSON.stringify(motion)}`);presentation='target height and reduced motion passed';}
   assert.equal(external,0);results.push({style,behavior:'passed',checks:['Park form serialization','event cancellation','busy state','confirmation focus/recovery','manual tabs','listbox selection','combobox selection/focus settlement','splitter keyboard resize and Home/End bounds','popover dismissal'],presentation,sizes});
  }catch(error){throw new Error(`Style ${style}: ${error.message}`,{cause:error});}finally{await context.close();}
 }
 console.log(JSON.stringify({version:1,recordedAt:new Date().toISOString(),node:process.version,sourceHashes:Object.fromEntries(['tools/ui-browser/check.mjs','tools/ui-browser/fixture/main.tsx','tools/ui-browser/fixture/index.html','tools/ui-browser/fixture/radical.css','packages/ui/src/styles/panda.css','packages/ui/src/park.tsx','packages/ui/src/components/ui/dialog.tsx'].map(path=>[path,createHash('sha256').update(readFileSync(join(root,path))).digest('hex')])),workingTreeStatus:execFileSync('git',['-C',root,'status','--short'],{encoding:'utf8'}).trim(),revision:execFileSync('git',['-C',root,'rev-parse','HEAD'],{encoding:'utf8'}).trim(),browser:browser.version(),scenario:'Real browser shared-control interaction independence with default/radical/no presentation CSS; synthetic local fixture, not application or theme66 acceptance.',results},null,2));
