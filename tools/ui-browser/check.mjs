...
try{
 ...
 console.log(JSON.stringify({version:1,recordedAt:new Date().toISOString(),node:process.version,sourceHashes:Object.fromEntries(['tools/ui-browser/check.mjs','tools/ui-browser/fixture/main.tsx','tools/ui-browser/fixture/index.html','tools/ui-browser/fixture/radical.css','packages/ui/src/styles/panda.css','packages/ui/src/workspace-region.tsx','packages/ui/src/dialog.tsx'].map(path=>[path,createHash('sha256').update(readFileSync(join(root,path))).digest('hex')])),workingTreeStatus:execFileSync('git',['-C',root,'status','--short'],{encoding:'utf8'}).trim(),revision:execFileSync('git',['-C',root,'rev-parse','HEAD'],{encoding:'utf8'}).trim(),browser:browser.version(),scenario:'Real browser shared-control interaction independence with default/radical/no presentation CSS; synthetic local fixture, not application or theme66 acceptance.',results},null,2));
