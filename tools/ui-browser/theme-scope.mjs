...
const source = {
  ...
  files: Object.fromEntries(['packages/shared/ui-theme.ts', 'packages/ui/src/theme.ts', 'packages/ui/src/theme-scope.ts', 'packages/ui/src/styles/panda.css', 'tools/ui-browser/theme-scope.mjs']
    .map(path => [path, createHash('sha256').update(readFileSync(join(root, path))).digest('hex')])),
};
// The synthetic scope needs a visible surface that consumes the same semantic
// variables as application layouts; the generated stylesheet supplies tokens.
const css = `${readFileSync(join(root, 'packages/ui/src/styles/panda.css'), 'utf8')}\n#first,#second{background-color:var(--colors-bg\\.surface);color:var(--colors-text\\.primary)}`;
...
button.addEventListener('click', () => {
  ...
  check(second.style.getPropertyValue('--tocyn-color-text') === '#edeef0', 'second container changed');
  ...
  check(getComputedStyle(first).backgroundColor === 'rgb(255, 255, 255)' && getComputedStyle(second).backgroundColor === 'rgb(17, 17, 19)', 'simultaneous rendered surfaces disagree');
  ...
  check(dark.surface === '#111113' && dark.text === '#edeef0', 'dark replacement failed');
  ...
  check(second.getAttribute('data-tocyn-theme-mode') === 'dark' && getComputedStyle(second).color === 'rgb(237, 238, 240)', 'removal changed another active scope');
  secondScope.remove();
...
const server = createServer((req, res) => {
  ...
  if (req.url === '/panda.css') { res.setHeader('Content-Type', 'text/css'); res.end(css); return; }
  if (req.url === '/bundle.js') { res.setHeader('Content-Type', 'text/javascript'); res.end(bundle); return; }
  res.setHeader('Content-Type', 'text/html'); res.end('<!doctype html><html><head><link rel="stylesheet" href="/panda.css"></head><body><main><button id="run">Run theme scope checks</button><div id="first" class="light" data-tocyn-workspace><input id="draft"></div><div id="second" class="light" data-tocyn-workspace></div><pre id="out"></pre></main><script type="module" src="/bundle.js"></script></body></html>');
});
...
try {
  ...
  const violations = []; const external = []; page.on('console', message => { if (message.type() === 'error') violations.push(message.text()); }); page.on('pageerror', error => violations.push(error.message));
  ...
  if (!output) throw new Error(`Theme fixture did not return a result: ${JSON.stringify({ violations, external })}`);
