...
- Remaining acceptance: full affected application/ShadowRoot browser and assistive-technology coverage, contrast/visual checks, widget numeric timing limits, authenticated interaction costs and exact integrated-revision validation. Login/bundle CI budgets now pass; built-widget timing and additional email VoiceOver receipts are recorded in `.agents/state/ui-48.md`. No issue closure or beta.2 readiness is inferred.
