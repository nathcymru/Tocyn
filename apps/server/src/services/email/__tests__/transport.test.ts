...
describe('local auth capture transport', () => {
  it('captures only the synthetic recipient with a bounded local login link', async () => {
    ...
    await capture.send({ to: [LOCAL_AUTH_CAPTURE_RECIPIENT], subject: 'Wrong local port', text: 'http://localhost:8787/verify?token=synthetic' }, capture.credentials);
    expect(capture.list().at(-1)?.loginLink).toBeUndefined();
    await expect(capture.send({ to: ['outside@example.com'], subject: 'No', text: 'No' }, capture.credentials)).rejects.toThrow(LOCAL_AUTH_CAPTURE_RECIPIENT);
    for (let index = 0; index < 10; index += 1) {
      await capture.send({ to: [LOCAL_AUTH_CAPTURE_RECIPIENT], subject: `Captured ${index}`, text: 'Synthetic local message' }, capture.credentials);
    }
    expect(capture.list()).toHaveLength(10);
    capture.reset();
    expect(capture.list()).toEqual([]);
    await capture.send({ to: [LOCAL_AUTH_CAPTURE_RECIPIENT], subject: 'Expiry', text: 'Synthetic local message' }, capture.credentials);
    now += 16 * 60 * 1000;
    expect(capture.list()).toEqual([]);
  });
});
