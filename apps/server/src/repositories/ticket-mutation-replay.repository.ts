...
export class TicketMutationReplayRepository {
  ...
  private async commitCanonical(candidate: MutationCandidate, ns?: MutationNamespace, staff?: StaffMutationCommit, api?: ApiMutationCommit): Promise<string> {
    ...
    const statements: D1PreparedStatement[] = [...(api ? apiBudgetMutationStatements(this.db,this.scope,api) : []), ...(staff ? staffMutationStatements(this.db,this.scope,staff) : []), ...(this.admission?.statements(operation)??[])];
    if (ns) {
