...
describe('Tenant-Scoped Repositories (Integration)', () => {
  ...
  it('prevents duplicate emails in the SAME tenant', async () => {
    ...
    await expect(reposA.users.create({ tenant_id: "default-tenant", email: 'dup@test.com', full_name: 'Dup 2', role: 'customer', mfa_enabled: false }))
      .rejects.toThrow(/UNIQUE constraint failed: users.tenant_id, users.email/);
  });

  it('allows duplicate emails across DIFFERENT tenants', async () => {
    await reposA.users.create({ tenant_id: "default-tenant", email: 'dup@test.com', full_name: 'Dup A', role: 'customer', mfa_enabled: false });
    await expect(reposB.users.create({ tenant_id: "default-tenant", email: 'dup@test.com', full_name: 'Dup B', role: 'customer', mfa_enabled: false })).resolves.toBeDefined();
  });
