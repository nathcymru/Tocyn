...
export const OPERATIONAL_METRICS_VERSION = '2026-09-11.2' as const;
export const OPERATIONAL_METRICS_FRESHNESS_MS = 60_000;
export const OPERATIONAL_METRICS_VISIBLE_TICKET_LIMIT = 1_000;
export const OPERATIONAL_METRICS_CANONICAL_EVENT_LIMIT = 1_000;
...
type BoundedCurrentWork = Readonly<{
  status: 'available' | 'truncated'; reason: 'visible_ticket_cap_exceeded' | null;
...
type BoundedCanonicalEvents = Readonly<{
  status: 'available' | 'truncated'; reason: 'visible_ticket_cap_exceeded' | 'canonical_event_cap_exceeded' | null;
...
export type OperationalMetricProjection = Readonly<{
  ...
  resourceLimits: Readonly<{ visibleTickets: number; canonicalEvents: number; assignees: number }>;
...
type ProjectionRow = { row_kind: 'summary' | 'assignee'; actor_authorized: number | null; visible_ticket_count: number | null; canonical_event_count: number | null; total: number | null; unassigned: number | null; latest_recorded_at: string | null; actor_id: string | null; assignee_count: number | null; };
type ProjectionLimits = Readonly<{ visibleTickets: number; canonicalEvents: number; assignees: number }>;
const DEFAULT_LIMITS: ProjectionLimits = Object.freeze({ visibleTickets: OPERATIONAL_METRICS_VISIBLE_TICKET_LIMIT, canonicalEvents: OPERATIONAL_METRICS_CANONICAL_EVENT_LIMIT, assignees: OPERATIONAL_METRICS_ASSIGNEE_LIMIT });
...
function resolveLimits(limits?: Partial<ProjectionLimits>): ProjectionLimits { return Object.freeze({ visibleTickets: boundedLimit(limits?.visibleTickets, DEFAULT_LIMITS.visibleTickets), canonicalEvents: boundedLimit(limits?.canonicalEvents, DEFAULT_LIMITS.canonicalEvents), assignees: boundedLimit(limits?.assignees, DEFAULT_LIMITS.assignees) }); }

/** One D1 statement rechecks authorization, group membership, caps, totals, and assignees. */
export class OperationalMetricsRepository {
  ...
  async currentWork(assigneeLimit = this.limits.assignees): Promise<OperationalMetricProjection> {
    ...
    const rows = await this.db.prepare(`WITH
      ...
      visible_ticket_boundary AS (
        SELECT t.id,t.assigned_to,t.status FROM tickets t
        WHERE EXISTS (SELECT 1 FROM authorized_actor) AND t.tenant_id=? AND (?=0 OR t.group_id IS NULL OR EXISTS (
      ...
      active_tickets AS (SELECT assigned_to FROM visible_ticket_boundary WHERE status IN ('open','pending')),
      visible_event_boundary AS (
        SELECT e.recorded_at FROM conversation_events e JOIN visible_ticket_boundary t ON t.id=e.ticket_id
      ...
      SELECT 'summary' AS row_kind,(SELECT count(*) FROM authorized_actor) AS actor_authorized,
        (SELECT count(*) FROM visible_ticket_boundary) AS visible_ticket_count,
        (SELECT count(*) FROM visible_event_boundary) AS canonical_event_count,
        (SELECT count(*) FROM active_tickets) AS total,
        (SELECT count(*) FROM active_tickets WHERE assigned_to IS NULL) AS unassigned,
        (SELECT max(recorded_at) FROM visible_event_boundary) AS latest_recorded_at,NULL AS actor_id,NULL AS assignee_count
      UNION ALL
      SELECT 'assignee',NULL,NULL,NULL,NULL,NULL,NULL,actor_id,assignee_count FROM assignee_boundary`)
      .bind(...authorizationValues, this.scope.tenantId, groupRestricted, this.scope.actorId, this.limits.visibleTickets + 1, this.scope.tenantId, this.limits.canonicalEvents + 1, assigneeLimit + 1)
      .all<ProjectionRow>();
    ...
    const ticketsTruncated = (summary.visible_ticket_count ?? 0) > this.limits.visibleTickets;
    const eventsTruncated = (summary.canonical_event_count ?? 0) > this.limits.canonicalEvents;
    ...
    return {
      ...
      currentWork: ticketsTruncated ? { status: 'truncated', reason: 'visible_ticket_cap_exceeded', denominator: 'tickets with current status open or pending visible to the current verified actor', total: null, unassigned: null, assigned: null, byAssignee: [], truncated: true } : { status: 'available', reason: null, denominator: 'tickets with current status open or pending visible to the current verified actor', total: summary.total ?? 0, unassigned: summary.unassigned ?? 0, assigned: (summary.total ?? 0) - (summary.unassigned ?? 0), byAssignee: assignees.slice(0, assigneeLimit).map(row => ({ actorId: row.actor_id, count: row.assignee_count })), truncated: assignees.length > assigneeLimit },
      canonicalEvents: ticketsTruncated ? { status: 'truncated', reason: 'visible_ticket_cap_exceeded', denominator: 'persisted canonical conversation events visible through the same tenant and group boundary', total: null, latestRecordedAt: null, lateOrReplayed: 'not-derived' } : eventsTruncated ? { status: 'truncated', reason: 'canonical_event_cap_exceeded', denominator: 'persisted canonical conversation events visible through the same tenant and group boundary', total: null, latestRecordedAt: null, lateOrReplayed: 'not-derived' } : { status: 'available', reason: null, denominator: 'persisted canonical conversation events visible through the same tenant and group boundary', total: summary.canonical_event_count ?? 0, latestRecordedAt: summary.latest_recorded_at, lateOrReplayed: 'not-derived' },
