...
export type SlaQueueRawRow=Readonly<{ticket_json:string;clock_json:string|null;pauses_json:string}>;
...
const boundedFields=['tickets.id','tickets.assigned_to','tickets.group_id'].map(field=>`length(CAST(COALESCE(${field},'') AS BLOB))<=256`)
 ...
 .map(field=>`length(CAST(COALESCE(${field},'') AS BLOB))<=64`)).concat(['tickets.ticket_no','tickets.status','tickets.priority','tickets.source','c.last_support_state_revision','c.revision','c.policy_revision','p.revision','c.policy_response_target_ms','c.policy_resolution_target_ms','p.response_target_ms','p.resolution_target_ms','c.policy_response_reopen_policy','c.policy_resolution_reopen_policy','c.pause_reason'].map(field=>`length(CAST(COALESCE(${field},'') AS BLOB))<=32`)).join(' AND ');
...
const textBytes=['tickets.ticket_no','tickets.id','tickets.subject','tickets.customer_email','tickets.status','tickets.priority','tickets.assigned_to','tickets.group_id','tickets.source','tickets.created_at','tickets.updated_at']
...
export class SlaPriorityQueueRepository {
 ...
 6*(${textBytes}+${latestSnippetBytes}+${clockValueBytes}+length(CAST(${effectiveCalendar} AS BLOB))+length(CAST(COALESCE(u.full_name,'') AS BLOB)))+1024+832*COALESCE(counter.pause_rows_upper_bound,0) AS byte_bound,
 ...
 async snapshot(commit:SlaQueueCommit):Promise<readonly SlaQueueRawRow[]>{
  ...
  const read=this.db.prepare(`${q.sql}, inputs AS MATERIALIZED (SELECT tickets.id,tickets.tenant_id,${ticketJson} AS ticket_json,${clockJson} AS clock_json
   ...
   SELECT inputs.ticket_json,inputs.clock_json,
