...
export const RETENTION_STEP_ENVELOPE: Readonly<ResourceAmounts> = Object.freeze({
  // Claim/recovery, one ownership lookup and one settled transition. The
  // envelope includes D1 index/trigger work rather than treating the three
  // application statements as the resource cost.
  workerRequests: 1, d1RowsRead: 2_560, d1RowsWritten: 32,
...
/** 100 claimed vector rows each transition through cleanup_claimed/cleaned. */
export const RETENTION_VECTOR_STEP_ENVELOPE: Readonly<ResourceAmounts> = Object.freeze({ ...RETENTION_STEP_ENVELOPE, d1RowsWritten: 256 });
/** One gated relational row plus trigger/cascade receipts, never a ticket-wide cascade. */
export const RETENTION_FINALIZE_STEP_ENVELOPE: Readonly<ResourceAmounts> = Object.freeze({ ...RETENTION_STEP_ENVELOPE, d1RowsWritten: 128 });
...
export async function admitRetentionStep(input: { env: Env; deps: TenantRequestDeps; ticketId: string; itemKey: string; attempt: number;
  resource: 'r2' | 'vector' | 'd1'; now: () => number }): Promise<RetentionAdmission> {
  ...
  try {
    const outcome = await apiTicketBudgetCache.admit({ repository: input.deps.repositories.budgetAuthority, namespace: input.env.BUDGET_COORDINATOR_DO,
      ...
      business: input.resource === 'r2' ? RETENTION_R2_STEP_ENVELOPE
        : input.resource === 'vector' ? RETENTION_VECTOR_STEP_ENVELOPE
          : input.resource === 'd1' ? RETENTION_FINALIZE_STEP_ENVELOPE : RETENTION_STEP_ENVELOPE,
