...
describe('diagnostic workload accounting contract', () => {
  it('includes each request, composition and retry in the existing resource dimensions', () => {
    ...
    expect(first).toEqual({ logEvents: 130, traceEvents: 0 });
    expect(sumResourceEnvelopes(first, retry, { d1RowsWritten: 4 })).toEqual({ logEvents: 196, traceEvents: 0, d1RowsWritten: 4 });
...
it('allows lower auth bounds only inside the trusted HTTP workload ceiling', () => {
  expect(estimateDiagnosticEnvelope({ httpRequests: 2, resourceCompositions: 0, appSessionAuthRequests: 0 })).toEqual({ logEvents: 2, traceEvents: 0 });
