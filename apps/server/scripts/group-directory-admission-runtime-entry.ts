...
function instrumentDatabase(db:any):any{
  ...
  const wrap=(raw:any,sql:string,values:any[]=[]):any=>{
    const proxy=new Proxy(raw,{get(target,property){
      ...
      if(property==='first')return async(column?:string)=>{
        // Runtime-only instrumentation executes the same prepared query via
        // native all(), exposing D1 metadata for the complete HTTP attempt.
        const result=await target.all();record(result.meta);const row=result.results?.[0]??null;
  ...
  wrappedDatabase=new Proxy(db,{get(target,property){
    ...
    if(property==='batch')return async(batch:any[])=>{
      const directory=batch.some(statement=>statements.get(statement)?.sql.includes('INSERT INTO budget_mutation_assertion'));
      if(directory&&beforeDirectory){
        const action=beforeDirectory;beforeDirectory=undefined;
        if(action.kind==='closure'){
          ...
          if(!link)throw new Error('Synthetic directory grant link was not prepared');
          ...
          await target.batch([
            ...
            target.prepare(`INSERT OR IGNORE INTO budget_grant_closures
              ...
              VALUES (?,?,?,?,?,?,1,'[]','[]',?)`).bind(tenantId,reservationId,holderId,aggregateId,`closed-${operationId}`,'synthetic-closed-set',Date.now()+60_000),
          ]);
        }
        if(action.kind==='session')await target.prepare("UPDATE users SET session_version=session_version+1 WHERE tenant_id='directory-tenant' AND id='00000000-0000-4000-8000-000000000001'").run();
        if(action.kind==='mfa')await target.prepare("UPDATE users SET mfa_enabled=0 WHERE tenant_id='directory-tenant' AND id='00000000-0000-4000-8000-000000000001'").run();
        if(action.kind==='capability')await target.prepare("UPDATE deployment_capability_ceiling SET enabled=0,revision=revision+1 WHERE capability='groups.manage'").run();
        if(action.kind==='users-growth')await target.prepare("INSERT INTO users(tenant_id,id,email,role,session_version,mfa_enabled) VALUES('directory-tenant','90000000-0000-4000-8000-000000000001','growth@example.test','agent',0,0)").run();
        if(action.kind==='groups-growth')await target.prepare("INSERT INTO groups(tenant_id,id,name) VALUES('directory-tenant','90000000-0000-4000-8000-000000000002','Growth group')").run();
        if(action.kind==='members-growth'&&action.groupId){
          await target.batch([
            target.prepare("INSERT OR IGNORE INTO users(tenant_id,id,email,role,session_version,mfa_enabled) VALUES('directory-tenant','90000000-0000-4000-8000-000000000003','member-growth@example.test','customer',0,0)"),
            target.prepare("INSERT INTO user_groups(tenant_id,user_id,group_id) VALUES('directory-tenant','90000000-0000-4000-8000-000000000003',?)").bind(action.groupId),
          ]);
        }
        if(action.kind==='budget')await target.prepare("UPDATE budget_owner_policies SET policy_json=policy_json||' ' WHERE deployment_id='directory-deployment'").run();
        if(action.kind==='ticket'&&action.groupId)await target.prepare("INSERT INTO tickets(tenant_id,id,subject,customer_email,group_id,source) VALUES('directory-tenant','growth-ticket','Race ticket','race@example.test',?,'dashboard')").bind(action.groupId).run();
      }
...
export default {async fetch(request:Request,env:any,ctx:ExecutionContext):Promise<Response>{
  ...
  if(path==='/__group-directory-control'){
    if(request.method==='POST'){
      const control=await request.json() as {kind:string;groupId?:string;discard?:boolean};
      if(control.discard)apiTicketBudgetCache.discardForTrustedRuntime();
      if(control.kind&&control.kind!=='discard')beforeDirectory=control;
    }
