...
test('presentation preferences are tenant/actor-scoped, version-safe, CAS-safe and fenced on revocation', async () => {
  await withTwoTenantFixture(async fixture => {
    ...
    assert.equal((await fixture.request(path, { token })).status, 503, 'Unknown schema is not silently converted into editable defaults');
    assert.equal((await save(0, {})).status, 409, 'Unknown schema cannot be overwritten with revision zero');
    await fixture.db.prepare('PRAGMA ignore_check_constraints=OFF').run();
    assert.equal((await save(0, {})).status, 409, 'Unsupported stored schema stays protected after constraint enforcement resumes');
    assert.equal((await save(2, { version: 1 })).status, 400);
    assert.equal((await save(Number.MAX_SAFE_INTEGER, {})).status, 400);
