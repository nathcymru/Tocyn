...
test('measures real fixture-authenticated dashboard and portal ticket list-to-detail navigation', async () => {
  const receipt = await withTwoTenantFixture(fixture => runAuthenticatedNavigation(fixture, Number(process.env.TOCYN_UI_AUTH_SAMPLES ?? '20'), {warmSwitches: process.env.TOCYN_UI_AUTH_WARM === '1'}));
