...
test('proves production composer versioned markdown, literal plain text, safe preview and keyboard paths', async () => {
  await withTwoTenantFixture(async fixture => {
    ...
    await initializeLocalBetaFixture(fixture, { runId:'composer-browser', tenants:[fixture.principals.customerA.tenantId, fixture.principals.customerB.tenantId], invitations:Object.values(fixture.principals).map(p => ({ tenantId:p.tenantId, id:p.localId, kind:p.role === 'customer' ? 'customer' as const : 'staff' as const })), limits:{ticketLimit:1,mutationLimit:12,recoveryReserve:2,uploadLimit:2} });
      ...
      const navigation = await page.goto(`${server.origin}/inbox/all/fixture-ticket`); assert.equal(navigation?.headers()['content-security-policy'],csp);
      ...
      const download = (status:number) => page.waitForResponse(response => new URL(response.url()).pathname.includes('/attachments/') && new URL(response.url()).pathname.endsWith('/download') && response.status() === status);
      server.failDownloadOnce(); const firstDownload = download(503); const preview = page.getByRole('button',{name:'Preview image synthetic-image.png',exact:true}); await preview.click(); await page.getByText('Image preview could not be loaded.',{exact:false}).waitFor(); const firstResponse = await firstDownload; const first = {status:firstResponse.status(),type:firstResponse.headers()['content-type'] ?? null,bytes:(await firstResponse.body()).byteLength};
      const secondDownload = download(200); await page.getByRole('button',{name:'Retry image preview synthetic-image.png',exact:true}).click(); const image = page.getByAltText('Preview of synthetic-image.png',{exact:true}); await image.waitFor(); const secondResponse = await secondDownload; const second = {status:secondResponse.status(),type:secondResponse.headers()['content-type'] ?? null,bytes:(await secondResponse.body()).byteLength};
      assert.equal(await image.evaluate(node => { const image = node as unknown as { complete: boolean; naturalWidth: number; naturalHeight: number }; return image.complete && image.naturalWidth === 1 && image.naturalHeight === 1; }),true,'The authenticated inline image must decode as the uploaded 1×1 PNG'); assert.deepEqual([first,second].map(download => [download.status,download.type,download.bytes]), [[503,'application/json',48],[200,'image/png',png.byteLength]],'The protected download retry must return the exact PNG bytes after one classified failure'); const hide = page.getByRole('button',{name:'Hide image preview synthetic-image.png',exact:true}); await hide.waitFor(); await hide.click(); assert.equal(await image.count(),0); const restoredPreview = page.getByRole('button',{name:'Preview image synthetic-image.png',exact:true}); await restoredPreview.waitFor(); assert.equal(await restoredPreview.evaluate((node: HTMLElement) => (globalThis as unknown as { document: { activeElement: unknown } }).document.activeElement === node),true,'Hiding the inline image must retain focus on its control');
