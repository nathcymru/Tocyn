...
export async function startIsolatedSnoozeReview(input: { port: number; intervalWindowMs: number;
  ...
  try {
    ...
    // This specialist review runtime uses the same fully classified demo
    // matrix as the ordinary local Beta fixture before adding its due case.
    const bootstrap = await createLocalFixtureBootstrap(secrets, { priorityReview: true });
    await db.batch(splitSql(bootstrap.sql).map(sql => db.prepare(sql)));
    const now = Date.now();
    await db.batch(tenants.flatMap((tenant, index) => [
      db.prepare('INSERT INTO groups(tenant_id,id,name) VALUES(?,?,?)').bind(tenant, 'review-group', 'Review'),
      db.prepare(`INSERT INTO tickets(tenant_id,id,ticket_no,subject,customer_id,customer_email,group_id,source,
        ...
        VALUES(?,'due-review',901,'Synthetic snooze review','fixture-customer',?,'review-group','dashboard',
          'status-follow-up','isolated',0,0,0,?,'delta',1)`)
        .bind(tenant, tenant === tenants[0] ? 'tocyn-auth-test-a@example.invalid' : 'tocyn-auth-test-b@example.invalid',
          calculatePriorityScore('status-follow-up', 'isolated', 0)),
      db.prepare(`INSERT INTO articles(tenant_id,id,ticket_id,sender_id,sender_type,body,snippet,body_format,
        is_internal,intake_source) VALUES(?,'due-review-article','due-review','fixture-customer','customer',?,?,
        'plain',0,'dashboard')`).bind(tenant, index === 0
        ? 'I am still waiting for the delivery update. Please follow up when the reminder becomes due.'
        : 'I am still waiting for the account update. Please follow up when the reminder becomes due.', index === 0
        ? 'Still waiting for the delivery update; follow up when due.'
        : 'Still waiting for the account update; follow up when due.'),
    ]));
