...
-- calendars, pauses and reopen policy. Existing tickets are deliberately not
-- backfilled: their actual ingestion start and inactive history are unknown.
CREATE TABLE ticket_priority_clocks (
  ...
  stop_reason TEXT CHECK (stop_reason IS NULL OR stop_reason IN ('waiting','resolved')),
...
AFTER UPDATE OF definition_id,waiting_reason ON ticket_support_state
