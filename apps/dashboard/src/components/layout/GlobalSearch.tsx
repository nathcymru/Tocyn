...
function SearchSession({ shortcutsEnabled }: { shortcutsEnabled: boolean }) {
  ...
  return <div className="tocyn-global-search relative w-full min-w-0 max-w-md flex-1" onKeyDown={event => { if (event.key === 'Escape' && event.target !== input.current) { event.preventDefault(); invalidate(); input.current?.focus(); } }}>
    <label className="text-xs font-medium">Search result type <TocynSelect aria-label="Search result type" value={type} onChange={event => { invalidate(); setType(event.target.value); }} className="ml-1 rounded border border-slate-300 bg-white text-slate-900"><option value="tickets">Tickets</option><option value="knowledge">Knowledge</option><option value="customers">Customers</option></TocynSelect></label>
    <div className="relative"><TocynInput ref={input} type="text" maxLength={256} value={query} aria-label={type === 'tickets' ? 'Search all tickets (global shell)' : `Search ${type} (global shell)`} aria-describedby="global-ticket-search-scope" aria-keyshortcuts={shortcutsEnabled ? 'Control+K Meta+K' : undefined} placeholder={type === 'tickets' ? 'Search all authorised tickets...' : type === 'knowledge' ? 'Search authorised knowledge titles...' : 'Customer search unavailable'} onChange={event => { invalidate(); setQuery(event.target.value); }} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); void search(); } if (event.key === 'Escape') { event.preventDefault(); clear(); } }} className="w-full pr-24 py-2 rounded border border-slate-300" />
    <TocynButton type="button" aria-label={type === 'tickets' ? 'Clear global ticket search' : 'Clear global search'} disabled={!query && !message} onClick={clear} className="absolute right-2 top-1/2 -translate-y-1/2 rounded px-2 py-1 text-xs underline">Clear</TocynButton>
    {shortcutsEnabled && <span data-tocyn-focus-decoration="" aria-hidden="true" className="pointer-events-none absolute right-16 top-1/2 hidden -translate-y-1/2 text-[10px] font-semibold text-slate-500 sm:inline">⌘/Ctrl K</span>}</div>
    <p id="global-ticket-search-scope" className="sr-only">{type === 'tickets' ? 'Searches all tickets you are authorised to access.' : type === 'knowledge' ? 'Searches titles in the complete authorised knowledge list.' : 'Customer search is unavailable.'} {shortcutsEnabled ? 'Press Command or Control K to focus this search.' : ''} Filter this view is available in the Inbox.</p>
    {(type === 'customers' || busy || message || selected) && <div className="absolute left-0 top-full z-50 mt-1 max-h-[min(24rem,60vh)] w-full overflow-y-auto rounded border border-slate-200 bg-white p-3 shadow-lg">
    {type === 'customers' ? <p role="status" className="text-sm">Customer search is not available in this workspace.</p> : (busy || message) ? <p role="status" className="text-sm">{busy ? 'Searching authorised results…' : message}</p> : null}
    {results.length > 0 && <ul aria-label={type === 'tickets' ? 'Ticket search results' : 'Knowledge search results'} className="max-h-64 overflow-auto border border-slate-200 bg-white">{results.map(row => <li key={row.id}>{type === 'tickets' ? <Link to={`/inbox/all/${encodeURIComponent(row.id)}`} className="block p-2 underline">Open in All tickets: {row.title || 'Untitled ticket'}</Link> : <TocynButton type="button" onClick={event => { previewOpener.current = event.currentTarget; setSelected(row); }} className="p-2 text-left underline">{row.title || 'Untitled knowledge'}</TocynButton>}</li>)}</ul>}
    {selected && <section aria-label="Knowledge result preview" className="border border-slate-200 bg-white p-2"><h2 ref={previewHeading} tabIndex={-1} className="font-semibold focus-visible:outline">{selected.title}</h2><p>Status: {selected.status}</p><p>Category reference: {selected.category_id ?? 'Uncategorised'}</p><TocynButton type="button" onClick={() => { setSelected(null); if (previewOpener.current?.isConnected) previewOpener.current.focus(); }}>Close preview</TocynButton></section>}
    </div>}
  </div>;
}
