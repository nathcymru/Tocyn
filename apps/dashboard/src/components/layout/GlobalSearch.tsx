...
function SearchSession({ shortcutsEnabled }: { shortcutsEnabled: boolean }) {
  ...
  const search = async () => {
    ...
    } catch { if (current()) setMessage(type === 'tickets' ? 'Ticket search is unavailable. No partial results are shown.' : type === 'customers' ? 'Customer search is unavailable. No partial results are shown.' : 'Knowledge search is unavailable. The complete authorised list could not be loaded within its limits.'); }
  ...
  return <div className="tocyn-global-search-shell" onKeyDown={event => { if (event.key === 'Escape' && event.target !== input.current) { event.preventDefault(); invalidate(); input.current?.focus(); } }}>
    <label className="tocyn-global-search-type-label">Universal search <ParkSelect aria-label="Universal search result type" value={type} onChange={event => { invalidate(); setType(event.target.value); }} className="tocyn-form-control"><option value="tickets">Tickets</option><option value="knowledge">Knowledge</option><option value="customers">Customers</option></ParkSelect></label>
    <div className="tocyn-global-search-input-shell"><MagnifyingGlassIcon aria-hidden="true" className="tocyn-global-search-icon" /><ParkInput ref={input} type="text" maxLength={256} value={query} aria-label={type === 'tickets' ? 'Search all tickets (global shell)' : `Search authorised ${type} (global shell)`} aria-describedby="global-ticket-search-scope" aria-keyshortcuts={shortcutsEnabled ? 'Control+K Meta+K' : undefined} placeholder={type === 'tickets' ? 'Search all authorised tickets...' : type === 'knowledge' ? 'Search authorised knowledge titles...' : 'Search authorised customers...'} onChange={event => { invalidate(); setQuery(event.target.value); }} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); void search(); } if (event.key === 'Escape') { event.preventDefault(); clear(); } }} className="tocyn-global-search" />
    <ParkButton type="button" aria-label={type === 'tickets' ? 'Clear global ticket search' : 'Clear global search'} disabled={!query && !message} onClick={clear} className="tocyn-search-clear">Clear</ParkButton>
    ...
    {results.length > 0 && <ul aria-label={type === 'tickets' ? 'Ticket search results' : type === 'knowledge' ? 'Knowledge search results' : 'Customer search results'} className="tocyn-global-search-results">{results.map(row => <li key={row.id}>{type === 'tickets' ? <Link to={`/inbox/all/${encodeURIComponent(row.id)}`} className="tocyn-global-search-result-link">Open in All tickets: {row.title || 'Untitled ticket'}</Link> : <ParkButton type="button" onClick={event => { previewOpener.current = event.currentTarget; setSelected(row); }} className="tocyn-global-search-result-button">{row.title || (type === 'customers' ? 'Unnamed customer' : 'Untitled knowledge')}</ParkButton>}</li>)}</ul>}
