...
function SearchSession({ shortcutsEnabled }: { shortcutsEnabled: boolean }) {
  ...
  return <div className="tocyn-global-search-shell" onKeyDown={event => { if (event.key === 'Escape' && event.target !== input.current) { event.preventDefault(); invalidate(); input.current?.focus(); } }}>
    ...
    {results.length > 0 && <ul aria-label={type === 'tickets' ? 'Ticket search results' : 'Knowledge search results'} className="tocyn-global-search-results">{results.map(row => <li key={row.id}>{type === 'tickets' ? <Link to={`/inbox/all/${encodeURIComponent(row.id)}`} className="tocyn-global-search-result-link">Open in All tickets: {row.title || 'Untitled ticket'}</Link> : <ParkButton type="button" onClick={event => { previewOpener.current = event.currentTarget; setSelected(row); }} className="tocyn-global-search-result-button">{row.title || 'Untitled knowledge'}</ParkButton>}</li>)}</ul>}
