...
import { TocynButton, TocynCheckbox, TocynRadio, TocynSelect } from '@luminatick/ui/primitives';
...
export function OperatorThemeControl() {
  ...
  return <section aria-labelledby="appearance-title" data-tocyn-appearance>
    ...
    <fieldset disabled={busy}><legend className="sr-only">Theme mode</legend>
      {(['system', 'light', 'dark'] as const).map(mode => <label key={mode}><TocynRadio name="operator-theme-mode" value={mode} checked={theme.mode === mode} onChange={() => theme.updateMode(mode as OperatorThemeMode)} />{mode === 'system' ? 'Use system setting' : mode === 'light' ? 'Light' : 'Dark'}</label>)}
    </fieldset>
  ...
  return <fieldset><legend>Table columns</legend>
    {ordered.map(column => {
      ...
      return <div key={column} ref={element => { if (element) rowRefs.current.set(column, element); else rowRefs.current.delete(column); }} role="group" aria-label={`${column} table column`}>
        <label><TocynCheckbox checked={visible} disabled={column === 'reference'} onChange={event => { if (column !== 'reference') preferences.update({ tableColumns: event.target.checked ? [...preferences.tableColumns, column] : preferences.tableColumns.filter(item => item !== column) }); }} /> {column}</label>
...
export function OperatorPreferencesControl() {
  ...
  return <section aria-labelledby="workspace-preferences-title" data-tocyn-appearance data-tocyn-preferences>
    ...
    <label><TocynCheckbox checked={preferences.focusMode} onChange={event => preferences.update({ focusMode: event.target.checked })} /> Focus mode</label>
    ...
    <label><TocynCheckbox checked={preferences.shortcutsEnabled} onChange={event => preferences.update({ shortcutsEnabled: event.target.checked })} /> Enable search shortcut</label>
    ...
    <label><TocynCheckbox checked={preferences.advanceAfterResolve} onChange={event => preferences.update({ advanceAfterResolve: event.target.checked })} /> Advance after resolving a conversation</label>
