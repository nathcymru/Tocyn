...
const valid = (value: unknown): value is OperatorPreferences => {
  ...
  return row.version === OPERATOR_PREFERENCES_VERSION && Number.isSafeInteger(row.revision) && Number(row.revision) >= 0
    ...
    && typeof row.focusMode === 'boolean' && (row.motion === 'system' || row.motion === 'reduced' || row.motion === 'full')
    && Array.isArray(columns) && columns.length >= 1 && columns.length <= OPERATOR_TABLE_COLUMNS.length
    && columns.includes('reference') && new Set(columns).size === columns.length
    && columns.every(column => OPERATOR_TABLE_COLUMNS.includes(column as OperatorTableColumn))
