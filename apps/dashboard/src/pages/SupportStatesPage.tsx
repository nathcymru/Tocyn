...
import { ParkButton, ParkCheckbox, ParkEmptyState, ParkInput } from '@luminatick/ui/park';
...
export function SupportStatesPage() {
  ...
  if (user?.role !== 'admin') return <div><h1 className={css({"fontSize":"xl","fontWeight":"semibold","lineHeight":"tight","color":"text.default"})}>Support states</h1><p role="alert" className={css({"minW":0})}>Only administrators can manage support states.</p></div>;
  ...
  return <div className={css({"maxW":"6xl","mx":"auto","px":{"base":"4","md":"6"},"py":"6","display":"grid","gap":"6"})}>
    <div className={css({"display":"flex","alignItems":"center","justifyContent":"space-between","gap":"3","flexWrap":"wrap","mb":"6"})}><h1 ref={heading} tabIndex={-1} className={css({"fontSize":"xl","fontWeight":"semibold","lineHeight":"tight","color":"text.default"})}>Support states</h1><p className={css({"color":"text.muted","fontSize":"sm","lineHeight":"relaxed"})}>Name the operator workflow separately from the customer-facing label. State labels do not change access permissions.</p></div>
    {notice && <p role="status">{notice}</p>}{errorMessage && <p role="alert">{errorMessage}</p>}
    {error && states.length === 0 && <div role="alert">Could not load support states. <ParkButton type="button" onClick={() => void refetch()}>Retry loading support states</ParkButton></div>}
    <form onSubmit={save} className={css({"maxW":"6xl","mx":"auto","px":{"base":"4","md":"6"},"py":"6"})} aria-label={editing ? 'Edit support state' : 'Create support state'}>
      <h2 className={css({"fontSize":"xl","fontWeight":"semibold","lineHeight":"tight","color":"text.default"})}>{editing ? `Edit ${editing}` : 'Create support state'}</h2>
      ...
      <div className={css({"display":"flex","alignItems":"center","justifyContent":"space-between","gap":"3","flexWrap":"wrap"})}><ParkButton type="submit" aria-disabled={create.isPending || update.isPending} className={css({"display":"inline-flex","alignItems":"center","gap":"2"})}>{editing ? 'Save state' : 'Create state'}</ParkButton>{editing && <ParkButton type="button" onClick={reset}>Cancel edit</ParkButton>}</div>
    </form>
    {isLoading ? <ParkEmptyState title="Loading support states…" headingLevel={false} aria-busy="true" className={css({"py":"6"})} /> : <>{states.length === 0 ? <ParkEmptyState title="No support states found." description="Create a support state to define the operator workflow." headingLevel={false} /> : <ul className={css({"display":"grid","gap":"4"})} aria-label="Support state definitions">{states.map(state => <li key={state.id} className={css({"bg":"bg.surface","borderWidth":"1px","borderColor":"border.default","rounded":"lg","p":"4"})}>
      <div className={css({"display":"flex","alignItems":"center","justifyContent":"space-between","gap":"3","flexWrap":"wrap"})}><div className={css({"minW":0})}><h2 className={css({"fontWeight":"medium","color":"text.default"})}>{state.internal_label} {!state.is_active && <span className={css({"minW":0})}>(inactive)</span>}</h2><p className={css({"color":"text.muted","fontSize":"sm","lineHeight":"relaxed"})}>Customer label: {state.public_label} · Legacy lifecycle: {state.legacy_status}</p><p className={css({"color":"text.muted","fontSize":"sm","lineHeight":"relaxed"})}>{state.waiting_reason_required ? 'Waiting reason required' : 'Waiting reason optional'} · {state.next_action_required ? 'Next action required' : 'Next action optional'}</p></div><div className={css({"display":"flex","alignItems":"center","justifyContent":"space-between","gap":"3","flexWrap":"wrap"})}><ParkButton type="button" onClick={() => beginEdit(state)} className={css({"display":"inline-flex","alignItems":"center","gap":"2"})}>Edit {state.internal_label}</ParkButton>{state.is_active === 1 && state.is_compatibility_default === 0 && <ParkButton type="button" onClick={() => { setDeactivating(state.id); setReplacementId(''); setErrorMessage(''); }} className={css({"minW":0})}>Deactivate {state.internal_label}</ParkButton>}</div></div>
      {deactivating === state.id && <div className={css({"minW":0})}><p className={css({"color":"text.muted","fontSize":"sm","lineHeight":"relaxed","display":"inline-flex","alignItems":"center","gap":"2"})}>Active tickets must move to an active replacement; this cannot leave tickets without a state.</p><label className={css({"fontWeight":"medium","color":"text.default","display":"grid","gap":"1","fontSize":"sm"})}>Replacement state<DashboardSelect aria-label="Replacement state" autoFocus value={replacementId} onValueChange={setReplacementId} options={[{ value: '', label: 'Choose a replacement' }, ...states.filter(candidate => candidate.is_active === 1 && candidate.id !== state.id).map(candidate => ({ value: candidate.id, label: `${candidate.internal_label} (${candidate.legacy_status})` }))]} /></label><div className={css({"display":"flex","alignItems":"center","justifyContent":"space-between","gap":"3","flexWrap":"wrap"})}><ParkButton type="button" aria-disabled={deactivate.isPending} onClick={() => void confirmDeactivate(state)} className={css({"display":"inline-flex","alignItems":"center","gap":"2"})}>Remap and deactivate</ParkButton><ParkButton type="button" onClick={() => setDeactivating(null)}>Cancel</ParkButton></div></div>}
    </li>)}</ul>}{hasMore && <ParkButton type="button" onClick={() => void loadMore()} aria-disabled={isLoadingMore}>{isLoadingMore ? 'Loading more support states…' : 'Load more support states'}</ParkButton>}{isLoadMoreError && <p role="alert" className={css({"p":"3","rounded":"md","bg":"bg.subtle","color":"text.default"})}>Could not load more support states. Try again.</p>}</>}
  </div>;
}
