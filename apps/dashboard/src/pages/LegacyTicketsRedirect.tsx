...
function Redirect(){
  ...
  return <ParkEmptyState title="Opening Inbox" description={failed?'Your saved list could not be confirmed. Retry without losing the filter.':'Restoring your conversation view…'} headingLevel={1} role={failed?'alert':'status'} action={failed&&<ParkButton type="button" variant="outline" onClick={()=>void run()}>Retry Inbox</ParkButton>} />;
}
