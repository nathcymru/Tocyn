...
it('keeps the inbox current-view query out of global results, marks body-free draft results, and uses a shareable global query URL', async () => {
  ...
  expect(screen.getByRole('textbox', { name: 'Search all authorised tickets' })).toHaveValue('global query');
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Clear global ticket search' }));
  await waitFor(() => expect(ticketQueries.at(-1)?.get('search')).toBeNull());
  expect(ticketQueries.at(-1)?.get('filter_id')).toBe('open-filter');
  expect(screen.getByRole('textbox', { name: 'Search all authorised tickets' })).toHaveValue('');
...
it('applies a legacy search URL once without restoring it over a later operator edit', async () => {
  ...
  const search = await screen.findByRole('textbox', { name: 'Search all authorised tickets' });
