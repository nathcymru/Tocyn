...
it('retains entered values through a conflict and failed reload, then explicitly resubmits the fresh revision',async()=>{
 ...
 expect(limit.closest('[data-scope="number-input"][data-part="root"]')).toHaveClass('number-input__root');
 expect(limit.closest('[data-scope="number-input"][data-part="root"]')?.querySelector('[data-part="label"]')).toHaveClass('number-input__label');
...
it('keeps an out-of-range value visible and blocks saving rather than silently clamping it',async()=>{
 vi.mocked(dashboardApi.get).mockResolvedValue(row());
 render(<OperatorCapacityPanel userId="operator" editable/>);
 const limit=await screen.findByRole('spinbutton',{name:'Assignment limit (0–1000)'});
 await userEvent.clear(limit);
 await userEvent.type(limit,'1001');
 act(()=>limit.blur());
 expect(limit).toHaveValue('1001');
 expect(screen.getByRole('button',{name:'Save capacity'})).toBeDisabled();
