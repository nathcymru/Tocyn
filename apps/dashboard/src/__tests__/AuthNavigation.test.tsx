...
it('continues from actual login through MFA to the dashboard when the auth boundary remounts the router', async () => {
  ...
  const code = (await screen.findAllByRole('textbox', { name: /Authentication Code/ }))[0];
  expect(window.location.pathname).toBe('/login');
...
it('retains mandatory setup and action focus after an authenticated invalid code, then completes login', async () => {
  ...
  const codeCells = screen.getAllByRole('textbox', { name: /Authentication Code/ });
  ...
  expect(codeCells.map((cell) => (cell as HTMLInputElement).value).join('')).toBe('123456'); expect(submit).toHaveFocus();
  codeCells.forEach((cell) => fireEvent.change(cell, { target: { value: '' } }));
  await userEvent.type(codeCells[0], '654321');
  fireEvent.click(submit);
  await screen.findByRole('heading', { name: 'Dashboard ready' });
