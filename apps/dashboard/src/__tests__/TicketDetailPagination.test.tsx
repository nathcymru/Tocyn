...
describe('dashboard conversation pagination',()=>{
  it('uses the page cursor, retains prior messages and focus, and announces completion',async()=>{
    vi.mocked(dashboardApi.get).mockImplementation(async(path)=>({ ...ticket,articles:[path.includes('article_cursor=')?article('second','Later staff-visible reply'):article('first','Accepted initial message')],pagination:path.includes('article_cursor=')?{has_more:false,next_cursor:null}:{has_more:true,next_cursor:'opaque-cursor'} }) as never);
