...
it('keeps labelled authenticated email read-only and guards duplicate submissions with the existing request contract',async()=>{
 ...
 for(const name of ['Your Name','Email Address','Subject','Message']) expect(screen.getByRole('textbox',{name}).closest('.field__root')).toBeInTheDocument();
...
it('preserves failed content, announces the error and supports an explicit retry',async()=>{
 ...
 const error=await screen.findByRole('alert');expect(error).toHaveClass('alert__root');expect(error.querySelector('.alert__description')).toHaveTextContent('could not be confirmed');await waitFor(()=>expect(error).toHaveFocus());expect(screen.getByRole('textbox',{name:'Message'})).toHaveValue('Synthetic message');expect(screen.getByRole('button',{name:'Send Message'})).toBeEnabled();
