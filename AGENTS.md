...
Before treating the general allowance as delivery capacity, inspect any separately reported GPT-5.3-Codex-Spark limits. When available, route suitable bounded coding, tests, documentation and validation to subscription-authenticated standalone Codex CLI workers using explicit `-m gpt-5.3-codex-spark` and the lowest sufficient effort. Spark has its own usage limits; this does not establish independent allowances for every Work/Codex environment. Keep consequential security review and integration with an adequately capable coordinator. Record the actual route, model, effort and observed bucket; preserve the configured credential store and never silently fall back to API authentication. Respect worker sandbox denials and return changes to the integration owner instead of working around protected paths.
